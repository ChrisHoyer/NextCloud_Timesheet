...
function generateRecordList(recordlist){
	...
	$.each(recordlist.report, function (day_index, day_entity){
		...
		} else { overtime = ""; overtime_sign="none"; }

		// create table for current day entry day header
		var row_day = [];
		row_day = "<div class='timesheet_tablerowmerged timesheet-report-row-d" +  day_entity.day + " timesheet-report-row-e" +  day_entity.eventtype + "' >" + "<div class='timesheet-tablerow' >";
		...
		row_day	= row_day + "<div class='timesheet-report-day-header-overtime timesheet-value-" + overtime_sign +"'>" + overtime + "</div>";
		...
		if(day_entity.hasOwnProperty('records')) {
			...
			$.each(day_entity.records, function (record_index, record_entity){
				...
				var record_table_row = "<div class='timesheet-tablerowhover' >";
			
				// Generate first column of object
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-time'></div>";		
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-time'>" + record_entity.starttime + "</div>";
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-time'>" + record_entity.endtime + "</div>";
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-time'>" + record_entity.breaktime + "</div>";
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-time'>" + record_entity.recordduration + "</div>";		
				...
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-modify'>";
				
				if(recordlist.settings.signedoff == "0")
				{
					record_table_row += "<span class='timesheet-record-delete icon-delete' data-dbid=" + record_entity.id + "></span>";
				...
				// Description
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-long'>" + record_entity.description + "</div>";

				// Project
				record_table_row += "<div class='timesheet-tablecell timesheet-tablecolumn-long'>" + record_entity.assignedproject_name + "</div>";
				// End Table Row
				record_table_row += "</div>";
	...
	if(recordlist.summary.difference_duration_hours !=0 ){
		...
		} else { monthly_difference = ""; monthly_difference_sign="none"; }
	...
	if(recordlist.summary.total_overtime_hours !=0 ){
		...
		} else { total_difference = ""; total_difference_sign="none"; }
	...
	// Generate table row content
	report_row = "<div class='timesheet-tablerow'>";
	
	// Generate table report row content
	report_row += "<div id='timesheet-record-table-summary-leftfiller'></div>";	
	report_row += "<div id='timesheet-record-summary-workinghours'> <span>&Sigma;</span> " + recordlist.summary.total_duration + "</div>";
	report_row += "<div id='timesheet-record-summary-monthlyovertime' class='timesheet-value-" + monthly_difference_sign +"'>" + monthly_difference + "</div>";
	report_row += "<div id='timesheet-record-summary-totalovertime' class='timesheet-value-" + total_difference_sign +"'> Total: " + total_difference + "</div>";
...
function generateReport(reportlist){
		...
		TScontent_selection += "<select id='timesheet-header-selectionbox-year' name='year' class='timesheet-selectionbox' >";
		
		// Iterate all years
		$.each(reportlist.reports, function (record_year, report_month){
			if(record_year == current_date.getFullYear()){
				TScontent_selection += "<option selected='selected'>" + record_year + "</option>";		
			} else
				TScontent_selection += "<option>" + record_year + "</option>";
		});
				
		// End Table Row
		TScontent_selection += "</select><select id='timesheet-header-selectionbox-month' name='month'  class='timesheet-selectionbox' >";
		...
		$.each(reportlist.reports[current_date.getFullYear()], function (record_index, report_month){	
			...
			if(report_month == (current_date.getMonth()+1))
				TScontent_selection += "<option selected='selected'>" + timesheet_MonthNames[report_month-1] + "</option>";			
			else
				TScontent_selection += "<option>" + timesheet_MonthNames[report_month-1] + "</option>";
		});
		
		TScontent_selection += "</select></div>";
...
function generateProject(projectlist){
		...
		// Generate table row content
		Project_NewRecord = "<select id='timesheet-newrecord-selectionbox-project' name='project' class='timesheet-selectionbox' >";	
		...
		Project_NewRecord = Project_NewRecord + "</select>";
